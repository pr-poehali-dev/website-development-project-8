...
import { cmsGetContent, type CmsContent } from "@/lib/cms";
...
const About = () => {
  const [content, setContent] = useState<Pick<CmsContent, "about_title" | "about_subtitle" | "about_description"> | null>(null);

  useEffect(() => {
    cmsGetContent().then((d) =>
      setContent({ about_title: d.about_title, about_subtitle: d.about_subtitle, about_description: d.about_description })
    );
  }, []);
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="mb-10 md:mb-16">
            <p className="text-xs uppercase tracking-widest text-muted-foreground mb-3">
              {content?.about_subtitle ?? "О нашей мастерской лазерной маркировки"}
            </p>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight mb-6">
              {content?.about_title ?? "Точность. Надёжность. Качество."}
            </h1>
            <p className="text-base md:text-lg text-muted-foreground leading-relaxed max-w-2xl">
              {content?.about_description ?? "Мы специализируемся на глубокой и поверхностной гравировке с использованием оптоволоконного лазера мощностью 30 Вт."}
            </p>
          </div>
